...
Vector rand_unit_vec(std::size_t size) {
    Vector unit(size);
    return unit;
}

// init is a minimised vector of atoms
// f is their force object
template <typename T> void find_saddle(Vector const &init, T const &f) {
    Vector x = init;
    Vector ax = rand_unit_vec(init.size());
...
int main() {
    ...
    for (int i = 0; i < 500; ++i) {
        ...
        if (dimer.findSaddle()) {
            ...
            if (d1 < 0.1) {
                dumpXYX(
                    std::string{"/home/cdt1902/dis/CATkS/plt/dump/colour_"} +
                        std::to_string(frame++) + std::string{".xyz"},
                    minus, f.colour(minus));
